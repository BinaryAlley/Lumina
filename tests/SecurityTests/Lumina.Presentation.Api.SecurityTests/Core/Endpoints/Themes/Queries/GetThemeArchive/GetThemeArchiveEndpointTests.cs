...
public class GetThemeArchiveEndpointTests : IClassFixture<LuminaApiFactory>, IDisposable
{
    ...
    public GetThemeArchiveEndpointTests(LuminaApiFactory apiFactory)
    {
    ...
    [Fact]
    public async Task GetThemeArchive_WhenUnauthorized_ShouldReturnUnauthorizedResult()
    {
        // Arrange
        string url = $"/api/v1/themes/{Guid.NewGuid():N}/archive";
    ...
    public async Task GetThemeArchive_WithPathTraversalInThemeId_ShouldReturnCleanNotFoundProblemDetails(string maliciousThemeId)
    {
        // Arrange
        HttpClient client = _apiFactory.CreateClient();
        await _apiFactory.CreateAndAuthenticateAdminUserAsync(client);
        string url = $"/api/v1/themes/{maliciousThemeId}/archive";
