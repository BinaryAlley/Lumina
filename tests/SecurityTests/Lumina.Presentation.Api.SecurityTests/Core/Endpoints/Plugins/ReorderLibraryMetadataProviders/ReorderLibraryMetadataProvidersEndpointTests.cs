...
public class ReorderLibraryMetadataProvidersEndpointTests : IClassFixture<LuminaApiFactory>
{
    ...
    private async Task SeedConfigurationAsync(Guid libraryId, int rank)
    {
        using IServiceScope scope = _apiFactory.Services.CreateScope();
        LuminaDbContext dbContext = scope.ServiceProvider.GetRequiredService<LuminaDbContext>();
        dbContext.LibraryMetadataProviderConfigurations.Add(_libraryMetadataProviderConfigurationEntityFixture.Create(libraryId: libraryId, pluginId: Guid.NewGuid(), rank: rank));
        await dbContext.SaveChangesAsync();
    }
}
