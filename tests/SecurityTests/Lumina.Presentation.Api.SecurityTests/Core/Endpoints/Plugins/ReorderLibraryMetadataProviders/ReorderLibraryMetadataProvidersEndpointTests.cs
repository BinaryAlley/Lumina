...
namespace Lumina.Presentation.Api.SecurityTests.Core.Endpoints.Plugins.ReorderLibraryMetadataProviders;

/// <summary>
/// Contains security tests for the <c>/libraries/{libraryId}/metadata-providers/reorder</c> route.
/// </summary>
[ExcludeFromCodeCoverage]
public class ReorderLibraryMetadataProvidersEndpointTests : IClassFixture<LuminaApiFactory>
{
    ...
    public ReorderLibraryMetadataProvidersEndpointTests(LuminaApiFactory apiFactory)
    {
    ...
    [Fact]
    public async Task ReorderLibraryMetadataProviders_WhenUnauthorized_ShouldReturnUnauthorizedResult()
    {
        // Arrange
        ...
        Assert.Equal($"/api/v1/libraries/{libraryId}/metadata-providers/reorder", problemDetails["instance"].GetProperty("value").GetString());
    ...
    public async Task ReorderLibraryMetadataProviders_WithSQLInjectionInPluginIds_ShouldNotLeakOrError(string maliciousPluginId)
    {
        // Arrange
        // authenticate with an admin user and seed a library with metadata provider configurations, so valid ids would reach the handler
        ...
        await SeedConfigurationAsync(libraryId, userId, rank: 1);
        await SeedConfigurationAsync(libraryId, userId, rank: 2);
        ...
        HttpResponseMessage response = await client.PutAsJsonAsync(
            $"/api/v1/libraries/{libraryId}/metadata-providers/reorder",
            new { libraryId = libraryId, pluginIds = new[] { maliciousPluginId } });
    ...
    private async Task SeedConfigurationAsync(Guid libraryId, Guid userId, int rank)
    {
        using IServiceScope scope = _apiFactory.Services.CreateScope();
        LuminaDbContext dbContext = scope.ServiceProvider.GetRequiredService<LuminaDbContext>();
