...
public class ScanLibrariesEndpointTests : IClassFixture<LuminaApiFactory>
{
    ...
    public ScanLibrariesEndpointTests(LuminaApiFactory apiFactory)
    {
    ...
    [Fact]
    public async Task ScanLibraries_WhenUnauthorized_ShouldReturnUnauthorizedResult()
    {
        // Act
        ...
        Assert.Equal("/api/v1/libraries/scans", problemDetails["instance"].GetProperty("value").GetString());
        Assert.Equal("Authentication failed", problemDetails["detail"].GetString());
    }

    [Fact]
    public async Task ScanLibraries_WhenAuthenticated_ShouldNotLeakSensitiveData()
    {
        // Arrange
        ...
        // Act
        HttpResponseMessage response = await client.PostAsJsonAsync("/api/v1/libraries/scans", new { });
