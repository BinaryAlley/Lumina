...
public class GetLibraryScanProgressEndpointTests : IClassFixture<LuminaApiFactory>
{
    ...
    public GetLibraryScanProgressEndpointTests(LuminaApiFactory apiFactory)
    {
    ...
    [Fact]
    public async Task GetLibraryScanProgress_WhenUnauthorized_ShouldReturnUnauthorizedResult()
    {
        // Arrange
        ...
        Assert.Equal($"/api/v1/libraries/{libraryId}/scans/{scanId}/progress", problemDetails["instance"].GetProperty("value").GetString());
    ...
    public async Task GetLibraryScanProgress_WithSQLInjectionInRouteParameters_ShouldNotLeakOrError(string maliciousId)
    {
        // Arrange
        ...
        // Act
        HttpResponseMessage response = await client.GetAsync($"/api/v1/libraries/{Uri.EscapeDataString(maliciousId)}/scans/{Guid.NewGuid()}/progress");
