...
public class CancelLibrariesScanEndpointTests : IClassFixture<LuminaWebFactory>
{
    ...
    [Fact]
    public async Task CancelLibrariesScan_WhenCalledWithoutAuthentication_ShouldRedirectToLogin()
    {
        // Arrange
        _apiFactory.ApiClientStub.Reset();
        HttpClient client = WebTestHelpers.CreateAnonymousClient(_apiFactory);
        string antiforgeryToken = await WebTestHelpers.GetAntiforgeryTokenAsync(client);
        HttpRequestMessage cancelRequest = CreateCancelRequest();
        ...
        Assert.DoesNotContain(_apiFactory.ApiClientStub.PostRequests, postRequest => postRequest.Endpoint == "libraries/scans/cancel");
    }

    [Fact]
    public async Task CancelLibrariesScan_WhenCalledWithoutAntiforgeryToken_ShouldRejectRequest()
    {
        // Arrange
        _apiFactory.ApiClientStub.Reset();
    ...
    /// <summary>
    /// Builds the cancel request that posts an empty JSON body to the cancel libraries scan endpoint.
    /// </summary>
    /// <returns>The configured cancel request.</returns>
    private static HttpRequestMessage CreateCancelRequest()
    {
        HttpRequestMessage cancelRequest = new(HttpMethod.Post, "/en-us/libraries/manage/api-cancel-libraries-scan")
        {
