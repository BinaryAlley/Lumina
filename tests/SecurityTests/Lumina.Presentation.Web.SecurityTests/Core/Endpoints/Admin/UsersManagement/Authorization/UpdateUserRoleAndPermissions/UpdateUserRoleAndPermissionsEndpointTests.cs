...
public class UpdateUserRoleAndPermissionsEndpointTests : IClassFixture<LuminaWebFactory>
{
    ...
    [Fact]
    public async Task UpdateUserRoleAndPermissions_WhenCalledWithoutAuthentication_ShouldRedirectToLogin()
    {
        // Arrange
        ...
        {
            Content = JsonContent.Create(_updateUserRoleAndPermissionsRequestFixture.Create())
        };
        ...
        Assert.DoesNotContain(_apiFactory.ApiClientStub.PutRequests, putRequest => putRequest.Endpoint.StartsWith("auth/users/", StringComparison.OrdinalIgnoreCase));
    }

    [Fact]
    public async Task UpdateUserRoleAndPermissions_WhenCalledWithoutAntiforgeryToken_ShouldRejectRequest()
    {
        // Arrange
        _apiFactory.ApiClientStub.Reset();
        _apiFactory.ApiClientStub.AuthorizationResponse = _getAuthorizationResponseFixture.Create(userId: Guid.NewGuid(), role: "Admin", permissions: []);
    ...
    public async Task UpdateUserRoleAndPermissions_WithInjectionInRoleId_ShouldRemainSecure(string maliciousRoleId)
    {
        // Arrange
        _apiFactory.ApiClientStub.Reset();
        _apiFactory.ApiClientStub.AuthorizationResponse = _getAuthorizationResponseFixture.Create(userId: Guid.NewGuid(), role: "Admin", permissions: []);
        AuthenticatedWebClient webClient = await WebTestHelpers.CreateAuthenticatedClientAsync(_apiFactory);
        string jsonBody = $"{{\"userId\":\"{Guid.NewGuid()}\",\"roleId\":\"{maliciousRoleId}\",\"permissions\":[]}}";
        HttpRequestMessage updateRequest = new(HttpMethod.Put, "/en-us/admin/api-update-user-authorization")
        {
