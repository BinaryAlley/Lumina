...
public class ManageRolesViewEndpointTests : IClassFixture<LuminaWebFactory>
{
    ...
    [Fact]
    public async Task ManageRoles_WhenCalledWithoutAuthentication_ShouldRedirectToLogin()
    {
        // Arrange
        ...
        // Act
        HttpResponseMessage response = await client.GetAsync("/en-us/admin/manage-roles");

        // Assert
        Assert.Equal(HttpStatusCode.Redirect, response.StatusCode);
        Assert.Equal("http://localhost/en-us/auth/login", response.Headers.Location!.ToString());
        Assert.DoesNotContain(_apiFactory.ApiClientStub.GetEndpointsCalled, endpoint => endpoint == "auth/roles");
    }
    ...
    [Fact]
    public async Task ManageRoles_WhenCalledByAdminUser_ShouldReturnRolesPage()
    {
        // Arrange
        _apiFactory.ApiClientStub.Reset();
