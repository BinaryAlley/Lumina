...
public class SplitPathEndpointTests : IClassFixture<LuminaWebFactory>
{
    ...
    [Fact]
    public async Task SplitPath_WhenCalledByAuthenticatedUser_ShouldReturnPathSegmentsFromApi()
    {
        // Arrange
        _apiFactory.ApiClientStub.Reset();
        string path = @"C:\Users\test";
        PathSegmentDto[] expectedSegments =
        [
            _pathSegmentDtoFixture.Create(path: @"C:\"),
            _pathSegmentDtoFixture.Create(path: "Users"),
            _pathSegmentDtoFixture.Create(path: "test")
        ];
        string expectedEndpoint = $"path/split?path={Uri.EscapeDataString(path)}";
        _apiFactory.ApiClientStub.RegisterGetResponse(expectedEndpoint, expectedSegments);
        AuthenticatedWebClient webClient = await WebTestHelpers.CreateAuthenticatedClientAsync(_apiFactory);
        HttpRequestMessage getRequest = new(HttpMethod.Get, $"/path/api-split?path={Uri.EscapeDataString(path)}");
