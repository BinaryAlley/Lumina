...
public class ValidatePathEndpointTests : IClassFixture<AuthenticatedLuminaApiFactory>, IAsyncLifetime
{
    ...
    [Fact]
    public async Task ValidatePath_WhenCalledWithEmptyPath_ShouldReturnValidationProblemResult()
    {
        // Arrange
        string path = "";
        ...
        Assert.Equal("/api/v1/path/validate", problemDetails["instance"].GetString());
    ...
    [Fact]
    public async Task ValidatePath_WhenCancellationTokenIsCanceled_ShouldThrowTaskCanceledException()
    {
        // Arrange
