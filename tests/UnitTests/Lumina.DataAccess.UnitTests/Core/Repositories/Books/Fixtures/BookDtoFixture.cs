...
/// <summary>
/// Fixture class for the <see cref="BookDto"/> class.
/// </summary>
...
public class BookDtoFixture
{
    ...
    #region ====================================================================== CTOR =====================================================================================
    /// <summary>
    /// Initializes a new instance of the <see cref="BookDtoFixture"/> class.
    /// </summary>
    public BookDtoFixture()
    {
    ...
    #region ===================================================================== METHODS ===================================================================================
    /// <summary>
    /// Creates a random valid BookDto.
    /// </summary>
    ...
    public BookDto CreateBookDto()
    {
        ...
        return new Faker<BookDto>()
            ...
            .RuleFor(x => x.Tags, f => new HashSet<TagDto>(CreateTags(f.Random.Number(1, 5))))
            .RuleFor(x => x.Genres, f => new HashSet<GenreDto>(CreateGenres(f.Random.Number(1, 5))))
    ...
    private List<TagDto> CreateTags(int count)
    {
        return new Faker<TagDto>()
            .CustomInstantiator(f => new TagDto(f.Random.String2(f.Random.Number(1, 50))))
            .Generate(count);
    }

    private List<GenreDto> CreateGenres(int count)
    {
        return new Faker<GenreDto>()
            .CustomInstantiator(f => new GenreDto(f.Random.String2(f.Random.Number(1, 50))))
            .Generate(count);
    }
    ...
    private List<IsbnDto> CreateIsbns(int count)
    {
        return new Faker<IsbnDto>()
            .CustomInstantiator(f => new IsbnDto(CreateIsbn(f), f.PickRandom<IsbnFormat>()))
            .Generate(count);
    }
    ...
    private List<BookRatingDto> CreateBookRatings(int count)
    {
        return new Faker<BookRatingDto>()
    ...
    {
        _fixture.Register(() => new TagDto(_fixture.Create<string>()));
        _fixture.Register(() => new GenreDto(_fixture.Create<string>()));
        _fixture.Register(() => new IsbnDto(_fixture.Create<string>(), _fixture.Create<IsbnFormat>()));
