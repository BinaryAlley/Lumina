...
namespace Lumina.Presentation.Api.UnitTests.Core.Endpoints.Themes.Management.DeleteTheme;
...
{
    private readonly Application.Common.CQRS.ICommandHandler<DeleteThemeCommand, Result<Success>> _mockHandler;
    ...
    {
        _mockHandler = Substitute.For<Application.Common.CQRS.ICommandHandler<DeleteThemeCommand, Result<Success>>>();
        _sut = Factory.Create<DeleteThemeEndpoint>(_mockHandler);
    }
    ...
    public async Task ExecuteAsync_WhenSuccessful_ShouldReturnNoContentResult()
    {
        ...
        _mockHandler.HandleAsync(Arg.Any<DeleteThemeCommand>(), Arg.Any<CancellationToken>())
    ...
    public async Task ExecuteAsync_WhenHandlerReturnsForbiddenError_ShouldReturnProblemResult()
    {
        ...
        DeleteThemeRequest request = _deleteThemeRequestFixture.Create();
        CancellationToken cancellationToken = CancellationToken.None;
        Error expectedError = DomainErrors.Themes.LastBundledThemeCannotBeDeleted;
        _mockHandler.HandleAsync(Arg.Any<DeleteThemeCommand>(), Arg.Any<CancellationToken>())
            .Returns(expectedError);
        ...
        Assert.Equal("LastBundledThemeCannotBeDeleted", problemDetailsBody.Detail);
        Assert.Equal("https://tools.ietf.org/html/rfc9110#section-15.5.4", problemDetailsBody.Type);
        Assert.NotNull(problemDetailsBody.Extensions["traceId"]);
    }
    ...
    public async Task ExecuteAsync_WhenCalled_ShouldSendDeleteThemeCommandToHandler()
    {
        ...
        await _mockHandler.Received(1).HandleAsync(
            Arg.Is<DeleteThemeCommand>(command => command.ThemeId == request.ThemeId),
