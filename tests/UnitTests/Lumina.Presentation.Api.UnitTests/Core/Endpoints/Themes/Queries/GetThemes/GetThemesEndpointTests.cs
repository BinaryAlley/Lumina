...
namespace Lumina.Presentation.Api.UnitTests.Core.Endpoints.Themes.Queries.GetThemes;
...
{
    private readonly IQueryHandler<GetThemesQuery, Result<IReadOnlyList<ThemeResponse>>> _mockHandler;
    ...
    {
        _mockHandler = Substitute.For<IQueryHandler<GetThemesQuery, Result<IReadOnlyList<ThemeResponse>>>>();
        _sut = Factory.Create<GetThemesEndpoint>(_mockHandler);
    }

    [Fact]
    public async Task ExecuteAsync_WhenSuccessful_ShouldReturnOkResultWithThemes()
    {
        // Arrange
        CancellationToken cancellationToken = CancellationToken.None;
        IReadOnlyList<ThemeResponse> expectedResponse = _themeResponseFixture.CreateMany();
        _mockHandler.HandleAsync(Arg.Any<GetThemesQuery>(), Arg.Any<CancellationToken>())
        ...
        // Assert
        Ok<IReadOnlyList<ThemeResponse>> okResult = Assert.IsType<Ok<IReadOnlyList<ThemeResponse>>>(result);
    ...
    public async Task ExecuteAsync_WhenCalled_ShouldSendGetThemesQueryToHandler()
    {
        ...
        CancellationToken cancellationToken = CancellationToken.None;
        _mockHandler.HandleAsync(Arg.Any<GetThemesQuery>(), Arg.Any<CancellationToken>())
            .Returns(Result.From<IReadOnlyList<ThemeResponse>>([]));
        ...
        await _mockHandler.Received(1).HandleAsync(
            Arg.Is<GetThemesQuery>(query => query != null),
    ...
    public async Task ExecuteAsync_WhenCancellationRequested_ShouldCancelOperation()
    {
        ...
        _mockHandler.HandleAsync(Arg.Any<GetThemesQuery>(), Arg.Any<CancellationToken>())
            .Returns(callInfo => Task.Run(async () =>
            {
                ...
                return Result.From<IReadOnlyList<ThemeResponse>>([]);
