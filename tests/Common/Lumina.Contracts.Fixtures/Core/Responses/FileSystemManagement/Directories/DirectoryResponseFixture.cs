...
namespace Lumina.Contracts.Fixtures.Core.Responses.FileSystemManagement.Directories;
...
public class DirectoryResponseFixture
{
    ...
    /// </summary>
    /// <param name="itemCount">Optional. The number of children elements to create.</param>
    ...
    /// <param name="itemCount">The number of children elements to create for each element.</param>
    /// <returns>The created list.</returns>
    public List<DirectoryResponse> CreateMany(int count = 3, int itemCount = 3)
    {
        return [.. Enumerable.Range(0, count).Select(_ => Create(itemCount))];
    }
}
