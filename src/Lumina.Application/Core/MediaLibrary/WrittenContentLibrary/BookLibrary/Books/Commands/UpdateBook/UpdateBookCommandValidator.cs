...
/// <summary>
/// Validates the needed validation rules for <see cref="UpdateBookCommand"/>.
/// </summary>
public class UpdateBookCommandValidator : AbstractValidator<UpdateBookCommand>
{
    ...
    public UpdateBookCommandValidator()
    {
        ...
        RuleFor(command => command.OCLCNumber)
            .Matches(@"^(ocm\d{8}|ocn\d{9,}|on\d{10,}|\(OCoLC\)\d{8,}|\d{8,})$")
            .When(command => command.OCLCNumber is not null)
