...
public class GetLibraryEndpoint : BaseEndpoint<GetLibraryRequest, IResult>
{
    ...
    {
        // retrieve the currently authenticated user, and send it with the request
        string? user = User.FindFirstValue(ClaimTypes.NameIdentifier);
        ErrorOr<LibraryResponse> result = await _sender.Send(request.ToQuery(Guid.Parse(user!)), cancellationToken).ConfigureAwait(false);
