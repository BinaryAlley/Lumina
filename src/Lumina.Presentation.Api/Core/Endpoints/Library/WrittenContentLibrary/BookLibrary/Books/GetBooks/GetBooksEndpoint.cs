...
{
    private readonly IQueryHandler<GetBooksQuery, ErrorOr<IEnumerable<BookResponse>>> _getBooksQueryHandler;
    ...
    public GetBooksEndpoint(IQueryHandler<GetBooksQuery, ErrorOr<IEnumerable<BookResponse>>> getBooksQueryHandler)
    {
    ...
    {
        ErrorOr<IEnumerable<BookResponse>> result = await _getBooksQueryHandler.HandleAsync(request.ToQuery(), cancellationToken).ConfigureAwait(false);
