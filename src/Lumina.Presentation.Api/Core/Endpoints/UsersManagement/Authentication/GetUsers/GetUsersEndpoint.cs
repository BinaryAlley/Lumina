...
{
    private readonly IQueryHandler<GetUsersQuery, Result<IEnumerable<UserResponse>>> _getUsersQueryHandler;
    ...
    public GetUsersEndpoint(IQueryHandler<GetUsersQuery, Result<IEnumerable<UserResponse>>> getUsersQueryHandler)
    {
    ...
    {
        Result<IEnumerable<UserResponse>> result = await _getUsersQueryHandler.HandleAsync(new GetUsersQuery(), cancellationToken).ConfigureAwait(false);
