...
namespace Lumina.Presentation.Web.Core.Endpoints.Admin.Themes.RestoreTheme;

/// <summary>
/// API endpoint for the <c>/{culture}/admin/themes/api-restore-theme/{themeId}</c> route.
/// </summary>
public class RestoreThemeEndpoint : BaseEndpoint<RestoreThemeRequest, IResult>
{
    ...
    public RestoreThemeEndpoint(ThemeService themeService)
    {
    ...
    public override void Configure()
    {
        base.Configure();
        Verbs(Http.POST);
        Routes(WebRoutes.Themes.RESTORE_THEME);
        DontAutoTag();
    ...
    public override async Task<IResult> ExecuteAsync(RestoreThemeRequest request, CancellationToken cancellationToken)
    {
        ...
        await _themeService.RestoreThemeAsync(request.ThemeId, cancellationToken).ConfigureAwait(false);
        return JsonSuccess();
    }
}
